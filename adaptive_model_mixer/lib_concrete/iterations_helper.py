...
class IterationState:
    ...
    def initialise_parser():
        ...
        parser.add_argument('--lmtd-eps', type=float, default=0.0000001, help='Max absolute lmtd error. DEFAULT=0.0001')
        parser.add_argument('--bal-eps', type=float, default=0.0000001, help='Max absolute error for balancing bilinearities. DEFAULT=0.0001')
        parser.add_argument('--area-eps', type=float, default=0.0000001, help='Max absolute error for area bilinearities. DEFAULT=0.0001')
        parser.add_argument('--beta-eps', type=float, default=0.0000001, help='Max absolute error for area betas. DEFAULT=0.0001')
        parser.add_argument('-e', '--all-error', type=float, default=0.000001, help='Max absolute error for all approximations. DEFAULT=0.0001')
    ...
    ## NEW FEATURES ##
    def add_tangent_points(self, model):
        print("Adding tangent points")
        model.del_component('Reclmtd_gradient_points')
        model.add_component('Reclmtd_gradient_points', Set(model.HP, model.CP, model.ST, dimen=2, initialize = lambda model, i, j, k: self.declare_stream_tangent_points(model, i, j, k)))
        
        model.del_component('Reclmtd_cu_gradient_points')
        model.add_component('Reclmtd_cu_gradient_points', Set(model.HP, dimen=1, initialize= lambda model, i: self.declare_cu_tangent_points(model, i)))
        
        model.del_component('Reclmtd_hu_gradient_points')
        model.add_component('Reclmtd_hu_gradient_points', Set(model.CP, dimen=1, initialize= lambda model, j: self.declare_hu_tangent_points(model, j)))

        return model
    
    def add_T_breakpoints(self, model):

        model.del_component('Th_breakpoints')
        model.add_component('Th_breakpoints', Set(model.HP, model.ST, dimen=1, initialize = lambda model, i, k: self.declare_th_breakpoints(model, i, k)))
        
        model.del_component('Thx_breakpoints')
        model.add_component('Thx_breakpoints', Set(model.HP, model.CP, model.ST, dimen=1, initialize = lambda model, i, j, k: self.declare_thx_breakpoints(model, i, j, k)))
        
        model.del_component('Tc_breakpoints')
        model.add_component('Tc_breakpoints', Set(model.CP, model.K_Take_First_Stage, dimen=1, initialize = lambda model, j, k: self.declare_tc_breakpoints(model, j, k)))

        model.del_component('Tcx_breakpoints')
        model.add_component('Tcx_breakpoints', Set(model.HP, model.CP, model.ST, dimen=1, initialize = lambda model, i, j, k: self.declare_tcx_breakpoints(model, i, j, k)))
    ...
    def add_q_breakpoints(self, model):
        ...
        model.del_component('Q_hu_breakpoints')
        model.add_component('Q_hu_breakpoints', Set(model.CP, dimen=1, initialize = lambda model, j: self.declare_hu_q_breakpoints(model, j)))
    ...
    def safe_initial_breakpoints(self, model):
        ...
        self.add_initial_th_breakpoints(model)
        self.add_initial_thx_breakpoints(model)
        self.add_initial_tc_breakpoints(model)
        self.add_initial_tcx_breakpoints(model)

        self.add_initial_stream_q_points(model)
        self.add_initial_cu_q_points(model)
        self.add_initial_hu_q_points(model)

        self.add_initial_stream_tangent_points(model)
        self.add_initial_cu_tangent_points(model)
        self.add_initial_hu_tangent_points(model)
