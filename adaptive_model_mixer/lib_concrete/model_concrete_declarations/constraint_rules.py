...
def th_lower_rule(model, i, k):
    return model.th[i,k] >= sum(model.Th_breakpoints[i,k].at(n)*model.z_th[i,k,n] for n in range(1, len(model.Th_breakpoints[i,k])))

def th_upper_rule(model, i, k):
    return model.th[i,k] <= sum(model.Th_breakpoints[i,k].at(n+1)*model.z_th[i,k,n] for n in range(1, len(model.Th_breakpoints[i,k])))
...
def area_hu_mccor_convex_1_rule(model, j):
    ...
    return  model.area_hu[j] >= \
            model.U_hu[j]*(\
                ...
                sum(\
                    model.Q_hu_breakpoints[j].at(n)*model.var_delta_reclmtd_hu[j,n]\
...
def area_low_rule(model, i, j, k):
    if model.Beta == 1:
        ...
        area_lower = sum(model.z_area_beta[i,j,k,m]*model.Area_beta_breakpoints[i,j,k].at(m) for m in range(1,len(model.Area_beta_breakpoints[i,j,k])))
...
def area_up_rule(model, i, j, k):
    if model.Beta == 1:
        ...
        area_upper = sum(model.z_area_beta[i,j,k,m]*model.Area_beta_breakpoints[i,j,k].at(m+1) for m in range(1,len(model.Area_beta_breakpoints[i,j,k])))
...
def area_pow_beta_rule(model, i, j, k):
    if model.Beta == 1:
        ...
        aread_lower = sum(model.z_area_beta[i,j,k,m]*model.Area_beta_breakpoints[i,j,k].at(m) for m in range(1,len(model.Area_beta_breakpoints[i,j,k])))
        aread_exp_lower = sum(model.z_area_beta[i,j,k,m]*model.Area_beta_exp[i,j,k].at(m) for m in range(1,len(model.Area_beta_breakpoints[i,j,k])))
        gradient = sum(model.z_area_beta[i,j,k,m]*model.Area_beta_gradients[i,j,k].at(m) for m in range(1,len(model.Area_beta_breakpoints[i,j,k])))
...
def area_cu_low_rule(model, i):
    if model.Beta == 1:
        ...
        area_cu_lower = sum(model.z_area_cu_beta[i,m]*model.Area_cu_beta_breakpoints[i].at(m) for m in range(1,len(model.Area_cu_beta_breakpoints[i])))
...
def area_cu_up_rule(model, i):
    if model.Beta == 1:
        ...
        area_cu_upper = sum(model.z_area_cu_beta[i,m]*model.Area_cu_beta_breakpoints[i].at(m+1) for m in range(1,len(model.Area_cu_beta_breakpoints[i])))
...
def area_cu_pow_beta_rule(model, i):
    if model.Beta == 1:
        ...
        area_cu_lower = sum(model.z_area_cu_beta[i,m]*model.Area_cu_beta_breakpoints[i].at(m) for m in range(1,len(model.Area_cu_beta_breakpoints[i])))
        area_cu_exp_lower = sum(model.z_area_cu_beta[i,m]*model.Area_cu_beta_exp[i].at(m) for m in range(1,len(model.Area_cu_beta_breakpoints[i])))
        gradient = sum(model.z_area_cu_beta[i,m]*model.Area_cu_beta_gradients[i].at(m) for m in range(1,len(model.Area_cu_beta_breakpoints[i])))
...
def area_hu_low_rule(model, j):
    if model.Beta == 1:
        ...
        area_hu_lower = sum(model.z_area_hu_beta[j,m]*model.Area_hu_beta_breakpoints[j].at(m) for m in range(1,len(model.Area_hu_beta_breakpoints[j])))
...
def area_hu_up_rule(model, j):
    if model.Beta == 1:
        ...
        area_hu_upper = sum(model.z_area_hu_beta[j,m]*model.Area_hu_beta_breakpoints[j].at(m+1) for m in range(1,len(model.Area_hu_beta_breakpoints[j])))
...
def area_hu_pow_beta_rule(model, j):
    if model.Beta == 1:
        ...
        area_hu_lower = sum(model.z_area_hu_beta[j,m]*model.Area_hu_beta_breakpoints[j].at(m) for m in range(1,len(model.Area_hu_beta_breakpoints[j])))
        area_hu_exp_lower = sum(model.z_area_hu_beta[j,m]*model.Area_hu_beta_exp[j].at(m) for m in range(1,len(model.Area_hu_beta_breakpoints[j])))
        gradient = sum(model.z_area_hu_beta[j,m]*model.Area_hu_beta_gradients[j].at(m) for m in range(1,len(model.Area_hu_beta_breakpoints[j])))
